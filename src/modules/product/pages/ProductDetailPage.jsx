      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
                ...
                {mediaItems.map((item, i) => (
                  <button
                    ...
                    className={`flex-shrink-0 w-16 h-16 overflow-hidden border-2 transition-all ${
                      activeMedia === i ? 'border-accent' : 'border-transparent hover:border-gray-300'
                    }`}
                  >
                    {item.type === 'video'
                      ? (
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button onClick={handleAddToCart} disabled={adding}
              className={`flex-1 flex items-center justify-center gap-2 font-bold text-sm px-5 py-3 transition-all active:scale-95 disabled:opacity-60 ${
                added ? 'bg-accent-dark text-white' : 'bg-accent text-white hover:bg-accent-dark'
              }`}>
              {added
          ...
          <a href={`https://wa.me/573015097013?text=${waText}`}
            ...
            className="flex items-center justify-center gap-2 border-2 border-gray-200 px-5 py-3 text-sm font-bold text-gray-700 hover:border-accent hover:text-black transition-all">
          ...
          {[
            ...
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`px-5 py-3 text-sm font-semibold border-b-2 -mb-px transition-colors ${
                tab === t.key ? 'border-accent text-black' : 'border-transparent text-gray-400 hover:text-gray-700'
              }`}>
              {t.label}
            </button>
          ))}
                ...
                <>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="mt-3 w-full text-sm border-2 border-gray-200 px-3 py-2 focus:outline-none focus:border-accent transition-colors resize-none"
                    />
                    {errorResena && (
                    ...
                    <button
                      ...
                      className="mt-3 bg-accent text-white font-bold text-sm px-5 py-2.5 hover:bg-accent-dark transition-colors disabled:opacity-50"
                    >
                    ...
                    {editandoPregId === p.id ? (
                      <div className="ml-10">
                        <textarea
                          ...
                          rows={2}
                          className="w-full text-sm border-2 border-gray-200 px-3 py-2 focus:outline-none focus:border-accent transition-colors resize-none"
                        />
                        <div className="flex gap-2 mt-1.5">
                          <button onClick={() => handleGuardarEdicionPregunta(p.id)}
                            className="text-xs font-bold text-white bg-accent px-3 py-1.5 hover:bg-accent-dark transition-colors">
                            Guardar
                          </button>
